...
class Reactor {
    ...
    potentiallyInvalidateToken(currentUser, opts) {
        ...
        if (wantsToSkip) {
            log_1.default.info('[auth-invalidate] skipped invalidateToken');
            return;
        }
        authAPI
            .signOut({
            ...
            .then(() => {
            log_1.default.info('[auth-invalidate] completed invalidateToken');
        })
            .catch((e) => { });
    }
    signOut(opts) {
    ...
    _patchPresencePeers(roomId, edits) {
        ...
        const newSessions = (0, mutative_1.create)(sessions, (draft) => {
            for (let [path, op, value] of edits) {
                switch (op) {
                    case '+':
                        (0, object_1.insertInMutative)(draft, path, value);
                        break;
                    case 'r':
                        (0, object_1.assocInMutative)(draft, path, value);
                        break;
                    case '-':
                        (0, object_1.dissocInMutative)(draft, path);
                        break;
                }
            }
    ...
    _setPresencePeers(roomId, data) {
        ...
        this._presence = (0, mutative_1.create)(this._presence, (draft) => {
            (0, object_1.assocInMutative)(draft, [roomId, 'result', 'peers'], peers);
        });
    }
    // --------
    // Broadcast
